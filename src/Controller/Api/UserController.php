<?php

namespace App\Controller\Api;


use App\Controller\Base\BaseApiController;
use App\Entity\User;
use App\Form\Api\UserApiType;
use App\Service\UserService;
use FOS\RestBundle\Controller\Annotations as Rest;
use FOS\RestBundle\View\View;
use Nelmio\ApiDocBundle\Annotation\Security as ApiSecurity;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Serializer\Normalizer\NormalizerInterface;

class UserController extends BaseApiController
{
    /**
     * @var NormalizerInterface
     */
    protected $errorNormalizer;

    /**
     * @var UserService
     */
    private $userService;

    /**
     * UserController constructor.
     */
    public function __construct(UserService $userService, NormalizerInterface $errorNormalizer)
    {
        $this->userService = $userService;
        $this->errorNormalizer = $errorNormalizer;
    }

    /**
     * @Rest\Get("/api/users", name="users")
     */
    public function index()
    {
        return View::create($this->userService->findAll(), Response::HTTP_CREATED);
    }

    /**
     * Creates new Customer resource.
     *
     * @Rest\Post("/api/users")
     * @ApiSecurity(name="apiUser")
     * @ApiSecurity(name="apiToken")
     */
    public function postAction(Request $request): View
    {
        try {
            $form = $this->createForm(UserApiType::class, new User());
            $data = $this->getJsonDecodedFromRequest($request);
            // $data['created_by'] = $this->getUserIdFromToken($request);

            $form->submit($data);
            /*if (!$form->isValid()) {
                $response = $this->getErrors($this->errorNormalizer, $form, Response::HTTP_BAD_REQUEST);

                return View::create($response, Response::HTTP_BAD_REQUEST);
            }*/
            $this->userService->create($form->getNormData());

            return View::create($data, Response::HTTP_CREATED);
        } catch (\Exception $ex) {
            //Log exception
            return View::create($ex->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }

    /**
     * Get User.
     *
     * @Rest\Get("/api/users/{username}")
     *
     * @ApiSecurity(name="apiUser")
     * @ApiSecurity(name="apiToken")
     */
    public function getUserByUsername(Request $request)
    {
        $user = $this->userService->findByName($request->get('username'));

        return $this->view($user);
    }

    /**
     * @Rest\Post("/api/login")
     */
    public function postLoginAction(Request $request)
    {
        $data = $this->getJsonDecodedFromRequest($request);
        $user = $this->userService->login($data['username'], $data['password']);
        if (!$user) {
            return View::create('username or password incorrect', Response::HTTP_BAD_REQUEST);
        }

        return View::create($user, Response::HTTP_ACCEPTED);
    }

    /**
     * @Rest\Post("/api/users/changePassword")
     *
     * @return \FOS\RestBundle\View\View
     *
     * @ApiSecurity(name="apiUser")
     * @ApiSecurity(name="apiToken")
     */
    public function changePassword(Request $request)
    {
        $data = $this->getJsonDecodedFromRequest($request);
        $user = $this->userService->changePassword($data['username'], $data['oldpassword'], $data['newpassword']);
        if (!$user) {
            return View::create('username or password incorrect', Response::HTTP_BAD_REQUEST);
        }

        return View::create($user, Response::HTTP_ACCEPTED);
    }
}
